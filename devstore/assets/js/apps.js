    async function loadApps() {
      try {
        const localRes = await fetch('versions.json?v=' + Date.now());
        if (localRes.ok) {
    ...
    window.addEventListener('DOMContentLoaded', () => {
      const apiEl = document.getElementById('api-url-text');
      if (apiEl) apiEl.textContent = new URL('versions.json', window.location.href).href;
